...
if test -r $linux_tree/include/linux/ipipe.h; then
   ...
   exit 2
elif test x$rtdm_native = x1; then
    if ! grep -q PREEMPT_RT $linux_tree/kernel/Kconfig.preempt; then
	echo "$me: No real-time preemption support in $linux_tree" >&2
        exit 2;
    fi
    if test x$verbose = x1; then
...
if test x$rtdm_native = x1; then
    ...
    if ! grep -q 'rtdm/native' $linux_tree/init/Kconfig; then
	p='source "kernel/xenomai/skins/rtdm/native/Kconfig"'
	( echo ; echo $p ) | patch_append init/Kconfig
    fi

    if ! grep -q 'CONFIG_XENO_SKIN_RTDM' $linux_tree/kernel/Makefile; then
	p="obj-\$(CONFIG_XENO_SKIN_RTDM) += xenomai/skins/rtdm/native/"
	( echo ; echo $p ) | patch_append kernel/Makefile
    fi

    if ! grep -q 'CONFIG_XENO_SKIN_RTDM' $linux_tree/drivers/Makefile; then
	p="obj-\$(CONFIG_XENO_SKIN_RTDM) += xenomai/"
    ...
    patch_link r n include include/xenomai
